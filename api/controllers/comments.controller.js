...
export const deleteComment = async (req, res, next) => {
    try {
        ...
        if (comment.userId !== req.user.id && !req.user.isAdmin) {
            return next(
                errorHandler(403, 'No tienes permisos para eliminar este comentario')
            );
        }
...
export const getcomments = async (req, res, next) => {
    if(!req.user.isAdmin) return next(errorHandler(401, 'No tienes permisos para ver todas las noticias'))
    try {
